...
export function buildRenderCacheKey(request: RenderRequest): string {
...
export async function tryReuseRenderCache(input: {
  ...
  try {
    const cache = JSON.parse(await readFile(input.cachePath, "utf8")) as RenderCacheMetadata
...
export async function writeRenderCacheMetadata(
  cachePath: string,
