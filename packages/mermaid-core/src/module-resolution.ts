...
export async function resolveModuleFileUrl(relativePath: string): Promise<string> {
  const resolvedPath = await resolveLocalModulePath(relativePath);
  return pathToFileURL(resolvedPath).href;
}

export async function resolveLocalModulePath(relativePath: string): Promise<string> {
